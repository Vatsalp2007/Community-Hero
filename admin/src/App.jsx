...
function MobileBottomNav() {
  ...
  return (
    <nav className="md:hidden fixed bottom-0 left-0 w-full bg-white/20 backdrop-blur-xl border-t border-white/10 h-16 flex justify-around items-center px-4 z-50">
      {mobileNav.map(item => {
        ...
        return (
          <NavLink
            key={item.to}
            to={item.to}
            className={`flex flex-col items-center gap-1 ${isActive ? 'text-cyan-300' : 'text-white/50'}`}
          >
            <item.icon size={20} />
...
function ProtectedAdminRoute({ children, fullBleed = false }) {
  ...
  if (loading) return <div className="flex items-center justify-center h-screen bg-slate-900"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-cyan-400" /></div>;
  if (!user) return <Navigate to="/login" />;
  if (profile && !['officer', 'admin'].includes(profile.role)) {
    return <div className="flex items-center justify-center h-screen bg-slate-900 flex-col gap-4"><h1 className="text-2xl font-bold text-red-400">Access Denied</h1><p className="text-sm text-white/60">You need officer or admin privileges.</p></div>;
  }

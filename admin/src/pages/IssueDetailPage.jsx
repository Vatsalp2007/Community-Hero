...
export default function IssueDetailPage() {
  ...
  if (!issue) return <div className="flex items-center justify-center h-64"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-cyan-400" /></div>;
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <button
        onClick={() => navigate(-1)}
        className="flex items-center gap-1 text-white/50 hover:text-cyan-300 transition-all mb-lg"
      >
      ...
      <div className="glass-card p-lg rounded-xl mb-lg">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <h1 className="text-headline-md font-bold text-white/90">{issue.title}</h1>
        ...
        {issue.description && (
          <p className="text-body-md text-white/60 mt-4">{issue.description}</p>
        )}
        <div className="flex items-center gap-2 text-body-md text-white/60 mt-2">
          <MapPin size={14} /> {issue.address || 'No address'}
        </div>
      </div>

      <div className="glass-card p-lg rounded-xl mb-lg">
        <h3 className="font-semibold text-white/90 mb-4">Admin Actions</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <select
              ...
              className="w-full bg-white/10 border border-white/10 rounded-lg px-4 py-2 text-sm focus:ring-2 focus:ring-white/30 text-white"
            >
              {Object.entries(ISSUE_STATUSES).map(([k, v]) => <option key={k} value={k} className="bg-slate-800 text-white">{v.label}</option>)}
            </select>
          </div>
          <div>
            <label className="block text-sm text-white/50 mb-1">Department</label>
            <select
              value={department}
              onChange={(e) => setDepartment(e.target.value)}
              className="w-full bg-white/10 border border-white/10 rounded-lg px-4 py-2 text-sm focus:ring-2 focus:ring-white/30 text-white"
            >
              {Object.entries(DEPARTMENTS).map(([k, v]) => <option key={k} value={k} className="bg-slate-800 text-white">{v}</option>)}
            </select>
          </div>
        </div>
        <div>
          <label className="block text-sm text-white/50 mb-1">Resolution Note</label>
          <textarea
            value={note}
            onChange={(e) => setNote(e.target.value)}
            rows={3}
            className="w-full bg-white/10 border border-white/10 rounded-lg px-4 py-2 text-sm focus:ring-2 focus:ring-white/30 text-white placeholder:text-white/40"
      ...
      <div className="glass-card p-lg rounded-xl mb-lg">
        <h3 className="font-semibold text-white/90 mb-3">Timeline</h3>

...
export default function DashboardPage() {
  ...
  useEffect(() => {
    let cancelled = false;
    getDocs(collection(db, 'issues')).then(snap => {
      if (!cancelled) setIssues(snap.docs.map(d => ({ id: d.id, ...d.data() })));
    });
    ...
    return () => { cancelled = true; unsubIssues(); unsubUsers(); };
  }, [refreshKey]);
  ...
  return (
    <div className="w-full space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-white/90">Dashboard Overview</h1>
        ...
        <div className="flex gap-3 shrink-0">
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm font-semibold whitespace-nowrap transition-all ${
              last30Days
                ? 'bg-gradient-to-r from-[#1A56DB] to-[#06B6D4] text-white shadow-lg'
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 bg-gradient-to-r from-[#1A56DB] to-[#06B6D4] text-white rounded-lg text-sm font-semibold shadow-lg whitespace-nowrap"
          >
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 glass-card text-white/70 rounded-lg text-sm font-semibold whitespace-nowrap"
          >
        ...
        {statCards.map((card, idx) => (
          ...
          >
            <div className="flex justify-between items-start">
              <span className="text-white/50 text-sm font-semibold">{card.label}</span>
            ...
            <div className="flex items-baseline gap-2 flex-wrap">
              <span className="text-4xl sm:text-5xl font-extrabold text-white/90 leading-none">{card.value}</span>
              {card.trend && (
              ...
              {card.sub && (
                <span className="text-white/50 text-sm font-semibold opacity-60">{card.sub}</span>
              )}
            </div>
          </div>
        ))}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-2 glass-card p-6 rounded-xl overflow-hidden min-h-[420px] flex flex-col">
          <div className="flex items-center justify-between mb-4 shrink-0">
            ...
            <button
              ...
              className="text-cyan-300 text-sm font-semibold hover:underline whitespace-nowrap cursor-pointer"
            >
              View Analytics
            </button>
          </div>
          ...
          >
            ...
            >
              <div className="absolute inset-[14px] rounded-full bg-slate-900/80 backdrop-blur-md border border-white/10 flex flex-col items-center justify-center shadow-inner">
                <span className="text-4xl sm:text-5xl font-extrabold text-white/80 leading-none">{totalForDonut}</span>
                <span className="text-white/50 text-[10px] uppercase tracking-widest text-center px-1 font-semibold">Total Reports</span>
              </div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-1 gap-2">
              {categoryCounts.map(d => (
                <div key={d.key} className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-semibold text-white/60 whitespace-nowrap">{d.label}: {d.count}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="xl:col-span-1 glass-card p-6 rounded-xl overflow-hidden flex flex-col min-h-[420px]">
          <div className="flex items-center justify-between mb-4 shrink-0">
            <h3 className="text-xl font-semibold text-white/90">Recent Issues</h3>
            <span className="bg-cyan-400/10 text-cyan-300 text-xs px-2.5 py-[3px] rounded-full font-semibold whitespace-nowrap">Live</span>
          </div>
          <div className="flex-1 overflow-y-auto space-y-3 custom-scrollbar min-h-0">
            {recentIssues.length === 0 && (
              <p className="text-white/50 text-base text-center py-8 opacity-60">No issues yet</p>
            )}
            {recentIssues.map(issue => {
              const catColor = CATEGORY_COLORS[issue.category] || '#6B7280';
              const statusInfo = ISSUE_STATUSES[issue.status];
              return (
                <div key={issue.id} onClick={() => navigate(`/issues/${issue.id}`)} className="flex gap-3 p-3 rounded-xl hover:bg-white/10 transition-all cursor-pointer group">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1 flex-wrap">
                      {issue.status === 'verified' ? (
                        <span className="bg-cyan-400/10 text-cyan-300 px-2 py-[1px] rounded-full text-[10px] font-bold">Community Verified</span>
                      ) : issue.severity === 5 ? (
                        <span className="text-red-400 text-xs font-bold">Critical</span>
                      ) : (
                        <span className="text-white/50 text-xs">{statusInfo?.label || issue.status}</span>
                      )}
                      <span className="text-white/50 text-xs opacity-40 hidden sm:inline">•</span>
                      <span className="text-white/50 text-xs">{timeAgo(issue.createdAt)}</span>
                    </div>
                  </div>
                  <span className="material-symbols-outlined text-white/30 opacity-0 group-hover:opacity-100 transition-opacity self-center">chevron_right</span>
                </div>
              );
            })}
          </div>
